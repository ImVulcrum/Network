...
func (pq PriorityQueue) Print() {
	for i := 0; i < len(pq); i++ {
		if pq[i].previous_knot == nil {
			fmt.Println(pq[i].cKnot.inhalt, pq[i].distance, "nil")
		} else {
			fmt.Println(pq[i].cKnot.inhalt, pq[i].distance, pq[i].previous_knot.inhalt)
		}
	}
}
...
func (n *data) MoveByContent(content string) error {
	if len(n.current_knot.Connections) == 0 {
		return fmt.Errorf("the current knot has no connections")
	}

	var i int
	for i = 0; i < len(n.current_knot.Connections); i++ {
		if n.current_knot.Connections[i].ziel.inhalt == content {
			n.current_knot = n.current_knot.Connections[i].ziel
			return nil
		}
	}

	if i == len(n.current_knot.Connections) {
...
func (n *data) MoveByWeight(type_of_movement int) error {
	...
	if type_of_movement >= 0 {
		var i int
		for i = 0; i < len(n.current_knot.Connections); i++ {
			if n.current_knot.Connections[i].gewicht == type_of_movement {
				n.current_knot = n.current_knot.Connections[i].ziel
				return nil
			}
		}
		...
		var current_smallest int = n.current_knot.Connections[0].gewicht

		for i := 0; i < len(n.current_knot.Connections); i++ {
			if n.current_knot.Connections[i].gewicht < current_smallest {
				current_smallest = n.current_knot.Connections[i].gewicht
				index = i
			}
		}
	} else if type_of_movement == -2 {
		var current_highest int = n.current_knot.Connections[0].gewicht
		for i := 0; i < len(n.current_knot.Connections); i++ {
			if n.current_knot.Connections[i].gewicht > current_highest {
				current_highest = n.current_knot.Connections[i].gewicht
				index = i
			}
		}
	}
	n.current_knot = n.current_knot.Connections[index].ziel
	return nil
}
...
func (n *data) AddKnot(inhalt string, outgoing_con []*Connection, incoming_con []*Connection) error {
	...
	for i := 0; i < len(outgoing_con); i++ {
		knot.Connections = append(knot.Connections, outgoing_con[i])
	...
	for i := 0; i < len(incoming_con); i++ {
		...
		incoming_con[i].ziel.Connections = append(incoming_con[i].ziel.Connections, connection)
	}
...
func (knot *Knot) String() string {
	...
	for i := 0; i < len(knot.Connections); i++ {
		erg = erg + "   --"
		erg = erg + fmt.Sprint(knot.Connections[i].gewicht, "-->")
		erg = erg + fmt.Sprintln(knot.Connections[i].ziel.inhalt)
	}
	return erg
}
...
func (knot *Knot) DeleteConnectionByDestination(delition_destination *Knot) {
	...
	for i = 0; i < len(knot.Connections); i++ {
		if knot.Connections[i].ziel == delition_destination {
			break
		}
	}

	//delete function
	if i < len(knot.Connections) {
		knot.Connections[i] = knot.Connections[len(knot.Connections)-1]
		knot.Connections[len(knot.Connections)-1] = nil
		knot.Connections = knot.Connections[:len(knot.Connections)-1]
	}
}
...
func (n *data) Dijkstras(startknot *Knot) PriorityQueue {
	...
	for i := 0; i < len(n.knots); i++ {
		if n.knots[i] == startknot {
			table.AddElement(Item{cKnot: startknot, distance: 0, previous_knot: nil})
		} else {
			table.AddElement(Item{cKnot: n.knots[i], distance: -3, previous_knot: nil})
		}
	}

	for index := 0; index < len(table); index++ {
		n.SetCurrentKnot(table[index].cKnot)
		connections := n.current_knot.Connections

		for i := 0; i < len(connections); i++ {
			distance := table[table.GetIndexOfElementByKnot(n.GiveCurrentKnot())].distance
			//überprüfen ob kanten gewicht + gewicht von wo wir sind kleiner als das was schon in der tabelle steht

			ziel := table.GetIndexOfElementByKnot(connections[i].ziel)
			current_distant_to_knot := distance + connections[i].gewicht

			if table[ziel].distance == -3 || current_distant_to_knot < table[ziel].distance {
				table[ziel].previous_knot = n.GiveCurrentKnot()
				table.ChangeElement(ziel, current_distant_to_knot)
